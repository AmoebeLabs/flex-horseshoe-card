      ...
      ${e?H`<g class="sparkline-labels sparkline-labels--x" style="pointer-events:none;">
        ${r.map(((t,e)=>H`
          <text
            ...
            style=${mt({...s,"text-anchor":0===e?"start":e===r.length-1&&t.x>=this.Graph.drawArea.x+this.Graph.drawArea.width?"end":s["text-anchor"]})}
          >${t.label}</text>
        `))}
      </g>`:""}
      ${i?H`<g class="sparkline-labels sparkline-labels--y" style="pointer-events:none;">
        ${o.map((t=>H`
          <text
            ...
            x="${this.Graph.drawArea.x-h-ka.calculateSvgDimension(this.config.y_axis.labels.offset)}"
            y="${t.y}"
            style=${mt(a)}
          >${t.label}</text>
        `))}
