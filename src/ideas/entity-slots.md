...
FHS keeps all configured entities in one flat entities[] list. Layout items can use entity_index, but a large list becomes difficult to maintain when template-owned default_entities, local FHS inputs, dynamically selected entities, and extra card entities are combined.
...
Tools receive only the final flat index. This keeps actions, more-info, FHS input routing, sparkline history, formatting, and the graph engine unchanged.
...
entity: sensor.temperature keeps its current meaning. FHS resolves the entity id against the final flat list and stores the resulting flat index internally.
...
- More-info and FHS input actions target the same entities as before.
