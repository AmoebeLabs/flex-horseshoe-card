...
export const computeStateDisplaySingleEntity = (localize, stateObj, locale, entity, state) =>
  ...
  computeStateDisplayFromEntityAttributes(localize, locale, entity, stateObj.entity_id, stateObj.attributes, state !== undefined ? state : stateObj.state);

export const computeStateDisplay = (localize, stateObj, locale, entities, state) => {
  const entity = entities[stateObj.entity_id];

  return computeStateDisplayFromEntityAttributes(localize, locale, entity, stateObj.entity_id, stateObj.attributes, state !== undefined ? state : stateObj.state);
};

export const computeStateDisplayFromEntityAttributes = (localize, locale, entity, entityId, attributes, state) => {
  ...
  if (isNumericFromAttributes(attributes)) {
    // state is duration
    if (attributes.device_class === 'duration' && attributes.unit_of_measurement && UNIT_TO_MILLISECOND_CONVERT[attributes.unit_of_measurement]) {
      try {
  ...
  if (['date', 'input_datetime', 'time'].includes(domain)) {
    if (state !== undefined) {
      ...
      if (attributes.has_date && attributes.has_time) {
        date = new Date(attributes.year, attributes.month - 1, attributes.day, attributes.hour, attributes.minute);
  ...
  if (['button', 'input_button', 'scene', 'stt', 'tts'].includes(domain) || (domain === 'sensor' && attributes.device_class === 'timestamp')) {
    try {
