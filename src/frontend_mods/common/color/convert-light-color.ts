...
export const temperature2rgb = (temperature: number): [number, number, number] => {
  ...
  return [Math.round(temperatureRed(value)), Math.round(temperatureGreen(value)), Math.round(temperatureBlue(value))];
};
...
export const mired2kelvin = (miredTemperature: number) => (miredTemperature === 0 ? 1000000 : Math.floor(1000000 / miredTemperature));

export const kelvin2mired = (kelvinTemperature: number) => (kelvinTemperature === 0 ? 1000000 : Math.floor(1000000 / kelvinTemperature));

export const rgbww2rgb = (rgbww: [number, number, number, number, number], minKelvin?: number, maxKelvin?: number): [number, number, number] => {
