...
export const DOMAINS_WITH_FAVORITES: FavoritesDomain[] = ['light', 'cover', 'valve'];

export const isFavoritesDomain = (domain: string): domain is FavoritesDomain => DOMAINS_WITH_FAVORITES.includes(domain as FavoritesDomain);

export const shouldShowFavoriteOptions = (values?: FavoriteOptionValue | null): boolean => values == null || values.length > 0;

export const hasCustomFavoriteOptionValues = (values?: FavoriteOptionValue | null): boolean => values != null;
...
export const findBatteryEntity = <T extends { entity_id: string }>(hass: HomeAssistant, entities: T[]): T | undefined => {
  const batteryEntities = entities
    .filter((entity) => hass.states[entity.entity_id] && hass.states[entity.entity_id].attributes.device_class === 'battery' && batteryPriorities.includes(computeDomain(entity.entity_id)))
    .sort((a, b) => batteryPriorities.indexOf(computeDomain(a.entity_id)) - batteryPriorities.indexOf(computeDomain(b.entity_id)));
...
export const findBatteryChargingEntity = <T extends { entity_id: string }>(hass: HomeAssistant, entities: T[]): T | undefined =>
  entities.find((entity) => hass.states[entity.entity_id] && hass.states[entity.entity_id].attributes.device_class === 'battery_charging');
...
export const getExtendedEntityRegistryEntry = (hass: HomeAssistant, entityId: string): Promise<ExtEntityRegistryEntry> =>
  hass.callWS({
    type: 'config/entity_registry/get',
    entity_id: entityId,
  });

export const getExtendedEntityRegistryEntries = (hass: HomeAssistant, entityIds: string[]): Promise<Record<string, ExtEntityRegistryEntry>> =>
  hass.callWS({
    type: 'config/entity_registry/get_entries',
    entity_ids: entityIds,
  });

export const updateEntityRegistryEntry = (hass: HomeAssistant, entityId: string, updates: Partial<EntityRegistryEntryUpdateParams>): Promise<UpdateEntityRegistryEntryResult> =>
  hass.callWS({
...
export const sortEntityRegistryByName = (entries: EntityRegistryEntry[], language: string) => entries.sort((entry1, entry2) => caseInsensitiveStringCompare(entry1.name || '', entry2.name || '', language));
...
export const getAutomaticEntityIds = (hass: HomeAssistant, entity_ids: string[]) =>
