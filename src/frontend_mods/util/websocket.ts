...
export const callWS = <T>(connection: Connection, msg: MessageBase): Promise<T> => {
  ...
  if (_debugConnection) {
    response.then(
      (result) =>
        // eslint-disable-next-line no-console
        console.log(`⬇️ Received #${msg.id}`, result && typeof result === 'object' ? JSON.parse(JSON.stringify(result)) : result),
      // eslint-disable-next-line no-console
