...
export default class HorseshoeGaugeV2 {
  ...
  constructor(config, index, templates, cardId) {
    ...
    this.previousStatePathItems = [];
    this.stateAnimationIndex = 0;
  }

  setState(entity, entityConfig) {
    if (this.runtimeConfig && this.scale && this.geometry && Number.isFinite(this.value)) {
      this.previousStatePathItems = this.getStatePathItems();
      this.stateAnimationIndex += 1;
    }
  ...
  renderScale() {
    ...
    const scaleArcs = this.getScaleArcs();
  ...
  renderState() {
    const config = this.runtimeConfig;
    const statePathItems = this.getStatePathItems();

    const stateStyle = {
      transition: 'fill 2.5s ease-out, opacity 2.5s ease-out',
    ...
    const stateAnimation = {
      ...DEFAULT_STATE_ANIMATION,
      ...(config.horseshoe_state.animation ?? {}),
    };

    return svg`
      <g class="horseshoe-v2__state-layer">
        ${statePathItems.map((pathItem) => {
          const previousPathItem = this.previousStatePathItems.find((item) => item.key === pathItem.key);

          const arcBaseStyle = pathItem.arc.active === false ? scaleStyle : stateStyle;

          const arcStyle = {
            ...arcBaseStyle,
            ...(pathItem.arc.color ? { fill: pathItem.arc.color } : {}),
          };

          const animatePath = stateAnimation.enabled && previousPathItem?.path && previousPathItem.path !== pathItem.path;
          ...
          return pathItem.path
            ? svg`
                <path
                  ...
                  d=${pathItem.path}
                  style=${styleMap(arcStyle)}
                >
                  ${
                    animatePath
                      ? svg`
                        <animate
                          id="horseshoe-v2__state-animation-${this.index}-${this.stateAnimationIndex}-${pathItem.key}"
                          attributeName="d"
                          from=${previousPathItem.path}
                          to=${pathItem.path}
                          dur="${stateAnimation.duration}ms"
                          calcMode="spline"
                          keyTimes="0;1"
                          keySplines=${stateAnimation.easing === 'linear' ? '0 0 1 1' : '0.16 1 0.3 1'}
                          fill="freeze"
                        ></animate>
                      `
                      : svg``
  ...
  getStatePathItems() {
    const config = this.runtimeConfig;
    const stateArcs = this.getStateArcs();
  ...
  getStateArcs() {
    ...
    if (config.horseshoe_state.mode === 'segment') {
      return this.getMappedStateArcs();
    }

    if (config.bar_mode === 'bidirectional') {
      return this.getBidirectionalStateArcs();
    }

    return this.getNormalStateArcs();
  }

  getNormalStateArcs() {
    return this.getColorAwareStateArcs({
      arcRange: {
  ...
  getColorAwareStateArcs(options = {}) {
    ...
    if (strokeStyle === 'colorstopsegments') {
      return this.getColorStopStateArcs({
        arcRange: {
          fromAngle,
          toAngle,
        },
  ...
  getColorStopStateArcs(options = {}) {
    const { arcRange = {} } = options;

    const config = this.runtimeConfig;
    const colorStops = asArray(config.colorStops?.colors);
    const gap = Number(config.colorStops?.gap ?? 0);
    const stateArcs = [];
    ...
    for (let i = 0; i < colorStops.length - 1; i += 1) {
      ...
      const colorStopStartAngle = this.geometry.valueToAngle(stopA.value);
      const colorStopEndAngle = this.geometry.valueToAngle(stopB.value);
    ...
    if (stateArcs.length) {
      stateArcs[0].startCap = config.horseshoe_state.linecap.start;
      stateArcs[stateArcs.length - 1].endCap = config.horseshoe_state.linecap.end;
    }
