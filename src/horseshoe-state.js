...
export function normalizeRuntimeConfig(config, colorStopMode) {
  ...
  const bidirectionalGradient = show.horseshoe_style === 'minmaxgradient' && (barMode === 'bidirectional' || barMode === 'bidirectional_symmetrical' || barMode === 'bidirectional_linear');

  if (bidirectionalGradient && horseshoeScale.min < 0 && !colorStops.colors.some((colorStop) => Number(colorStop.value) < 0)) {
    throw new Error(`[V2] minmaxgradient with ${barMode} requires a color_stop below 0`);
  }

  if (bidirectionalGradient && horseshoeScale.max > 0 && !colorStops.colors.some((colorStop) => Number(colorStop.value) > 0)) {
    throw new Error(`[V2] minmaxgradient with ${barMode} requires a color_stop above 0`);
  }
