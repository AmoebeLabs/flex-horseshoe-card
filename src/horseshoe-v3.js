...
import { buildLabelStopItems } from './horseshoe-shapes.js';
...
export default class HorseshoeV3 extends BaseTool {
  ...
  static setConfig(config, templates, cardId, card) {
    ...
    return horseshoes
      ...
      .map((horseshoeConfig, index) => new HorseshoeV3(normalizeBaseConfig(horseshoeConfig, index, card.cardLayout.groupManager, card.cardTheme.getActiveColorStopMode()), index, templates, cardId, card))
  ...
  buildMeasuredGradientContracts() {
    ...
    if (rebuildPathElements) {
      ...
      const labels = labelStops.map((labelStop, index) => {
        ...
        const text = HorseshoeLabels.applyEllipsis(String(labelStop.text), this.config.horseshoe_labels.ellipsis);
  ...
  renderStateAtProgress(progress, pathId) {
    ...
    if (clip.end <= clip.start) {
      return renderNormalizedPathBands(this.pathDefinition, [], this.renderContract.stateLayer, `${pathId}-state`, 'horseshoe-v3__state-band');
    }
    ...
    if (this.stateGradient || this.currentStateGradientConfig) {
      ...
      return renderAdaptivePathGradient(this.pathDefinition, gradient, this.renderContract.stateLayer, `${pathId}-state-gradient`, 'horseshoe-v3__state-gradient');
    }
    ...
    return renderNormalizedPathBands(this.pathDefinition, ranges, this.renderContract.stateLayer, `${pathId}-state`, 'horseshoe-v3__state-band');
  }

  /** Binds the committed master centerline for gradients, tickmarks, labels, badges, and markers. */
  updated() {
    const pathId = `${this.cardId}-horseshoe-v3-${this.index}`;
    const masterPath = this.card.shadowRoot.querySelector(`#${pathId}-master`);
    const newlyBound = this.pathGeometry.bindPathElement(masterPath);
    const stateLayerElement = this.card.shadowRoot.querySelector(`#${pathId}-state`);
  ...
  render() {
    ...
    return this.renderItemLayers(svg`
      ...
      >
        <g class="horseshoe-v3__path" transform=${this.pathTransform}>
          <path
            id="${pathId}-master"
          ...
          ${this.backgroundLayers.map((background) =>
            background.gradient
              ? renderAdaptivePathGradient(background.definition, background.gradient, background.layer, `${pathId}-${background.id}-background-gradient`, `horseshoe-v3__${background.id}-background-gradient`)
              : renderNormalizedPathBands(background.definition, background.ranges, background.layer, `${pathId}-${background.id}-background`, `horseshoe-v3__${background.id}-background`),
          )}
          ${
            this.scaleGradient
              ? renderAdaptivePathGradient(this.pathDefinition, this.scaleGradient, this.renderContract.backgroundLayer, `${pathId}-scale-gradient`, 'horseshoe-v3__scale-gradient')
              : renderNormalizedPathBands(this.pathDefinition, this.renderContract.scaleRanges, this.renderContract.backgroundLayer, `${pathId}-scale`, 'horseshoe-v3__scale')
          }
        ...
        <g class="horseshoe-v3__ticks-labels-badges-markers">
